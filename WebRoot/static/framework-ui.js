        ...
        $.ajax({
            ...
            success: function (data) {
                ...
                $.each(data, function (i, value) {                    
                    ...
                    arrHtml.push(value[options.id]);
                    arrHtml.push('">');
                    arrHtml.push(value[options.text]);
                    arrHtml.push('</option>');
                });
                var str = arrHtml.join("");
                $element.html(str);
            }
        });
    }
}
$.fn.initDataItems = function (options) {
    var defaults = {
        Key: ""
    };
    var options = $.extend(defaults, options);
    var $element = $(this);
    if (options.Key != "") {
        var dataItems = top.clients.dataItems[options.Key];
        var arrHtml = [];
        $.each(dataItems, function (i, value) {
            arrHtml.push('<option value="');
            arrHtml.push(i);
            arrHtml.push('">');
            arrHtml.push(value);
...
$.fn.authorizeButton = function () {
    ...
    if (dataJson != undefined) {
        $.each(dataJson, function (i) {
            $element.find("#" + dataJson[i].BT_CODE).attr('authorize', 'yes');
        });
    }
...
function iDocPreview(libID) {
	link = "/appCadViewer/view.do?LIB_ID="+libID;
	window.open(link,'_self');
}
function iDocPreviewC(libID) {
	link = "/appCadViewer/view.do?LIB_ID="+libID;
	window.open(link,'_blank');
}
function iDocPreviewBlank(fileID) {
	var docServerIP = top.clients.dataItems.IDocViewIP;
	link = docServerIP+"/view/"+fileID;
	window.open(link,'_blank');
}
function iDocPreviewDownload(fileID) {
	var docServerIP = top.clients.dataItems.IDocViewIP;
    if(fileID==null||fileID==''){
    	$.modalMsg("请先选择文件", "warning");
    }else{
    	window.location.href=docServerIP+"/doc/download/"+fileID;
    }
}