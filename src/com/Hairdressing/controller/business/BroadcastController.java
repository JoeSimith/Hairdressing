...
import com.Hairdressing.service.business.BroadcastService;

/** 
 * 类名称：BroadcastController
 * 创建时间：2018-06-09
 */
@Controller
...
public class BroadcastController extends BaseController {
	...
	public String saveOrUpdate() throws Exception{
		logBefore(logger, "新增或编辑Broadcast");
		PageData pd = this.getPageData();
		if(pd.get("ID")==null||pd.get("ID").equals("")) {
			pd.put("ID", this.get32UUID()); // 主键
			this.broadcastService.save(pd);
		}else {
			this.broadcastService.edit(pd);
		}
	...
	public Object getGridListJson() throws Exception {
		logBefore(logger, "获取Broadcast列表Json");
		PageData pd = this.getPageData();
		Page page = new Page();
		page.setCurrentPage(pd.getInt("page"));
		page.setShowCount(pd.getInt("rows"));
		page.setPd(pd);
		List<PageData> resultList = this.broadcastService.listPage(page);// 分页查询列表
	...
	public Object getFormJson() throws Exception {
		...
		PageData pd = this.getPageData();
		PageData resultPD = this.broadcastService.findById(pd);
	...
	public ModelAndView list(Page page){
		logBefore(logger, "列表Broadcast");
		ModelAndView mv = this.getModelAndView();
		mv.setViewName("business/broadcast/list");
		return mv;
	}
	...
	public ModelAndView toAdd(){
		logBefore(logger, "去新增Broadcast页面");
		ModelAndView mv = this.getModelAndView();
		mv.setViewName("business/broadcast/form");
		return mv;
	}	
	...
	public ModelAndView toEdit(){
		logBefore(logger, "去修改Broadcast页面");
		ModelAndView mv = this.getModelAndView();
		mv.setViewName("business/broadcast/form");		
		return mv;
	}	
	...
	public ModelAndView toDetail(){
		logBefore(logger, "去查看Broadcast页面");
		ModelAndView mv = this.getModelAndView();
		mv.setViewName("business/broadcast/detail");		
		return mv;
	}
	...
	public Object deleteAll() {
		logBefore(logger, "批量删除Broadcast");
	...
	public ModelAndView exportExcel(){
		logBefore(logger, "导出Broadcast到excel");
		...
		try{
			...
			List<PageData> varOList = broadcastService.listAll(pd);
			List<PageData> varList = new ArrayList<PageData>();
			for(int i=0;i<varOList.size();i++){
				PageData vpd = new PageData();
				vpd.put("var1", varOList.get(i).get("ID").toString());	//1
				vpd.put("var2", varOList.get(i).getString("picpath"));	//2
				vpd.put("var3", varOList.get(i).getString("pictitle"));	//3
				vpd.put("var4", varOList.get(i).getString("piccontent"));	//4
				vpd.put("var5", varOList.get(i).getString("available"));	//5
